...
public class A04Main {
    ...
    private static void characterTable(final BST<Character> tree) {
    	int total = 0;
    	double percent = 0;
    	TreeNode<Character> new_node = tree.root;
    	ArrayList <CountedValue<Character>> list = tree.levelOrder();
    	
    	for(int j=0;j<list.size();j++) {
    		total+=list.get(j).getCount();
    	}
    	
    	System.out.println("Char    Count Percent");
    	for(int i=0;i<list.size();i++) {
    		percent = list.get(i).getCount()/total;
    		System.out.printf("\t%c%10d\t%f",list.get(i).getValue(),list.get(i).getCount(),percent);
    ...
    private static void fillTree(final BST<Character> tree, final String string) {
    	...
    	TreeNode<Character> node = tree.root;
    	
    	for(int i=0;i<string.length();i++) {
    		if(Character.isLetter(string.charAt(i))){
    			CountedValue<Character> value = new CountedValue<>(Character.toUpperCase(string.charAt(i)));
    			TreeNode<Character> new_node = new TreeNode<Character>(value);
        		node = new_node;
        	}
    		node = node.getRight();
    	}
    ...
    private static int retrieve(final BST<Character> tree, final Scanner fileScan) { 
    	...
    	while(fileScan.hasNext()) {
    		CountedValue<Character> letter = new CountedValue<Character>(Character.valueOf(fileScan.next().charAt(0)));
    		if(tree.contains(letter)) {
    			comparisons+=1;
    		}
    		//CountedValue<Character> value = new CountedValue<>(letter);
			//TreeNode<Character> node = tree.retrieve(value);
    		//TreeNode<CountedValue> node = tree.retrieve(letter);
    	}
